...
use \block_timetable\privacy\provider;
...
class block_timetable_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences($type, $value, $expected) {
        ...
        provider::export_user_preferences($user->id);
        $writer = writer::with_context(\context_system::instance());
        $blockpreferences = $writer->get_user_preferences('block_timetable');
        if (!$expected) {
            $expected = get_string($value, 'block_timetable');
        }
        $this->assertEquals($expected, $blockpreferences->{$type}->value);
    }

    /**
     * Create an array of valid user preferences for the timetable block.
     *
     * @return array Array of valid user preferences.
     */
    public function user_preference_provider() {
        return array(
            array('block_timetable_user_sort_preference', 'sortbydates', ''),
            array('block_timetable_user_sort_preference', 'sortbycourses', ''),
            array('block_timetable_user_sort_preference', 'next7days', ''),
            array('block_timetable_user_sort_preference', 'all', ''),
            array('block_timetable_user_limit_preference', 5, 5),
        );
    }
}
